...
{
    public partial class CdAssetPropertiyEditorUC : XtraUserControl
    {
        #region - Var -
        private static readonly log4net.ILog Logger = log4net.LogManager.GetLogger(typeof(CdAssetPropertiyEditorUC));
        ...
        private void UOW_BeforeCommitTransaction(object sender, DevExpress.Xpo.SessionManipulationEventArgs e)
        {
            // Get NewId
            int id = 0;
            object obj = Classes.Managers.DataManager.GetNewId("CdAssetPropertiy", "AssetPropertiyId");
            if (obj == null)
            {
            ...
            {
                if (item.GetMemberValue("AssetPropertiyId") == null)
                {
                    item.SetMemberValue("AssetPropertiyId", id);
                    id++;
                }
            }
        }
