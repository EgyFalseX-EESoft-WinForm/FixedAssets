...
{
    public partial class CDComponentsEditorUC : XtraUserControl
    {
        #region - Var -
        private static readonly log4net.ILog Logger = log4net.LogManager.GetLogger(typeof(CDComponentsEditorUC));
        ...
        private void UOW_BeforeCommitTransaction(object sender, DevExpress.Xpo.SessionManipulationEventArgs e)
        {
            // Get NewId
            int id = 0;
            object obj = Classes.Managers.DataManager.GetNewId("CDComponents", "ComponentId");
            if (obj == null)
            {
            ...
            {
                if (item.GetMemberValue("ComponentId") == null)
                {
                    item.SetMemberValue("ComponentId", id);
                    id++;
                }
            }
        }
