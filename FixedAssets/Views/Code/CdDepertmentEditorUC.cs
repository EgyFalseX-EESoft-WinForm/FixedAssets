...
{
    public partial class CdDepertmentEditorUC : XtraUserControl
    {
        #region - Var -
        private static readonly log4net.ILog Logger = log4net.LogManager.GetLogger(typeof(CdDepertmentEditorUC));
        Datasource.dsData.RoleDetialRow _elementRule = null;
        #endregion
        #region - Fun -
        public CdDepertmentEditorUC(Datasource.dsData.RoleDetialRow RuleElement)
        {
            InitializeComponent();
        ...
        private void UOW_BeforeCommitTransaction(object sender, DevExpress.Xpo.SessionManipulationEventArgs e)
        {
            // Get NewId
            int id = 0;
            object obj = Classes.Managers.DataManager.GetNewId("CdDepertment", "DepertmentId");
            if (obj == null)
            {
            ...
            {
                if (item.GetMemberValue("DepertmentId") == null)
                {
                    item.SetMemberValue("DepertmentId", id);
                    id++;
                }
            }
        }
