...
{
    public partial class CDUnitEditorUC : XtraUserControl
    {
        #region - Var -
        private static readonly log4net.ILog Logger = log4net.LogManager.GetLogger(typeof(CDUnitEditorUC));
        Datasource.dsData.RoleDetialRow _elementRule = null;
        #endregion
        #region - Fun -
        public CDUnitEditorUC(Datasource.dsData.RoleDetialRow RuleElement)
        {
            InitializeComponent();
        ...
        private void UOW_BeforeCommitTransaction(object sender, DevExpress.Xpo.SessionManipulationEventArgs e)
        {
            // Get NewId
            int id = 0;
            object obj = Classes.Managers.DataManager.GetNewId("CDUnit", "UnitId");
            if (obj == null)
            {
            ...
            {
                if (item.GetMemberValue("UnitId") == null)
                {
                    item.SetMemberValue("UnitId", id);
                    id++;
                }
            }
        }
