...
{
    public partial class CDInsuranceCoEditorUC : XtraUserControl
    {
        #region - Var -
        private static readonly log4net.ILog Logger = log4net.LogManager.GetLogger(typeof(CDInsuranceCoEditorUC));
        Datasource.dsData.RoleDetialRow _elementRule = null;
        #endregion
        #region - Fun -
        public CDInsuranceCoEditorUC(Datasource.dsData.RoleDetialRow RuleElement)
        {
            InitializeComponent();
        ...
        private void UOW_BeforeCommitTransaction(object sender, DevExpress.Xpo.SessionManipulationEventArgs e)
        {
            // Get NewId
            int id = 0;
            object obj = Classes.Managers.DataManager.GetNewId("CDInsuranceCo", "InsuranceCoID");
            if (obj == null)
            {
            ...
            {
                if (item.GetMemberValue("InsuranceCoID") == null)
                {
                    item.SetMemberValue("InsuranceCoID", id);
                    id++;
                }
            }
        }
