...
{
    public partial class CdAssetplaceEditorUC : XtraUserControl
    {
        #region - Var -
        private static readonly log4net.ILog Logger = log4net.LogManager.GetLogger(typeof(CdAssetplaceEditorUC));
        Datasource.dsData.RoleDetialRow _elementRule = null;
        #endregion
        #region - Fun -
        public CdAssetplaceEditorUC(Datasource.dsData.RoleDetialRow RuleElement)
        {
            InitializeComponent();
        ...
        private void UOW_BeforeCommitTransaction(object sender, DevExpress.Xpo.SessionManipulationEventArgs e)
        {
            // Get NewId
            int id = 0;
            object obj = Classes.Managers.DataManager.GetNewId("CdAssetplace", "AssetplaceId");
            if (obj == null)
            {
            ...
            {
                if (item.GetMemberValue("AssetplaceId") == null)
                {
                    item.SetMemberValue("AssetplaceId", id);
                    id++;
                }
            }
        }
